...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      let parsedAnalysis = {
        detectedItems: { mattress: 0, couch: 1, appliance: 0, tv_monitor: 0, tire: 0, yard_bag: 3 },
        itemTags: [
...
Return ONLY a valid JSON object matching the requested schema with all required fields.`;
        ...
        if (geminiRes.ok) {
          ...
          if (rawText) {
            try {
              ...
              console.warn("Failed to parse Gemini JSON output, using structured estimate:", jsonErr);
            }
          }
        } else {
          console.warn("Gemini call returned error status:", geminiRes.status);
        }
      }
      ...
      // 4. Shopify Storefront API Cart Creation (replaces Stripe checkout)
